...
export const Sidebar = ({ currentView, onNavigate }) => {
  ...
  return (
    <aside
      ...
      } bg-white border-r border-slate-200 flex flex-col shrink-0 h-full no-print shadow-xs select-none transition-[width] duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] relative will-change-[width]`}
    >
      {/* Floating Center Border Toggle Button */}
      <button
        type="button"
        onClick={toggleCollapse}
        className="hidden md:flex absolute top-1/2 -right-3 -translate-y-1/2 w-6 h-6 bg-white hover:bg-emerald-50 border border-slate-200 hover:border-emerald-400 rounded-full items-center justify-center text-slate-400 hover:text-emerald-600 shadow-xs hover:shadow-sm transition-all duration-200 z-20 cursor-pointer group hover:scale-105 active:scale-95"
